...
class InventoryLocationRepository extends BaseRepository
{
     ...
     * @return      InventoryLocation[]|LengthAwarePaginator
     */
    function where ($query, $ignorePagination = true, $maxLimit = 5000, $maxPage = 100)
    {
        ...
        $qb->select(['inventoryLocation', 'organization']);
    ...
    {
        $qb->from('App\Models\WMS\InventoryLocation', 'inventoryLocation')
            ->join('inventoryLocation.organization', 'organization', Query\Expr\Join::ON);

        if (!is_null(AU::get($query['ids'])))
            $qb->andWhere($qb->expr()->in('inventoryLocation.id', $query['ids']));
        ...
        if (!is_null(AU::get($query['barCodes'])))
        {
            ...
            {
                $orX->add($qb->expr()->eq('inventoryLocation.barCode', $qb->expr()->literal($barCode)));
            }
            $qb->andWhere($orX);
        }

        $qb->orderBy('inventoryLocation.id', 'ASC');
        return $qb;
    }
