...
class CartRepository extends BaseRepository
{
     ...
     * @return      Cart[]|LengthAwarePaginator
     */
    function where ($query, $ignorePagination = true, $maxLimit = 5000, $maxPage = 100)
    {
        ...
        $qb->select(['cart', 'organization']);
        $qb                         =   $this->buildQueryConditions($qb, $query);

        $qb->addOrderBy(AU::get($query['orderBy'], 'cart.id'), AU::get($query['direction'], 'ASC'));
    ...
    {
        $qb->from('App\Models\WMS\Cart', 'cart')
            ->join('cart.organization', 'organization', Query\Expr\Join::ON);

        if (!is_null(AU::get($query['ids'])))
            $qb->andWhere($qb->expr()->in('cart.id', $query['ids']));
        ...
        if (!is_null(AU::get($query['barCodes'])))
        {
            ...
            {
                $orX->add($qb->expr()->eq('cart.barCode', $qb->expr()->literal($barCode)));
            }
            $qb->andWhere($orX);
        }
