...
use App\Models\Locations\Continent;
...
class ContinentRepository extends BaseRepository
{
     ...
     * @return      Continent[]|LengthAwarePaginator
     */
    ...
    {
        $qb->from('App\Models\Locations\Continent', 'continent');

        if (!is_null(AU::get($query['ids'])))
            $qb->andWhere($qb->expr()->in('continent.id', $query['ids']));
        ...
        if (!is_null(AU::get($query['names'])))
        {
            ...
            {
                $orX->add($qb->expr()->LIKE('continent.name', $qb->expr()->literal('%' . trim($name) . '%')));
            }
            $qb->andWhere($orX);
        }
