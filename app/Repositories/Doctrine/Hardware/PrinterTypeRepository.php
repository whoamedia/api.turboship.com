...
class PrinterTypeRepository extends BaseRepository
{
     ...
     * @return      PrinterType[]|LengthAwarePaginator
     */
    ...
    private function buildQueryConditions(QueryBuilder $qb, $query)
    {
        ...
        if (!is_null(AU::get($query['names'])))
        {
            ...
            {
                $orX->add($qb->expr()->LIKE('printerType.name', $qb->expr()->literal('%' . trim($name) . '%')));
            }
            $qb->andWhere($orX);
        }

        $qb->orderBy('printerType.id', 'ASC');
        return $qb;
    }
