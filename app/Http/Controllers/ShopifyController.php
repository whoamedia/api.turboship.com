...
class ShopifyController extends BaseAuthController
{
    ...
    public function downloadProducts (Request $request)
    {
        ...
        if ($downloadShopifyProducts->getPendingSku() == true)
        {
            ...
            $job                            = (new DownloadShopifyProductsJob($this->integratedShoppingCart->getId(), true))->onQueue('shopifyBulkImports');
            $this->dispatch($job);
        }
        else
        {
            ...
            $job                            = (new DownloadShopifyProductsJob($this->integratedShoppingCart->getId(), false))->onQueue('shopifyBulkImports');
            $this->dispatch($job);
        }
